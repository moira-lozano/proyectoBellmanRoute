    ...
    <div class="row" v-else>
      <div class="col-12">
        <chart-card title="Ordenes de Clientes Frecuentes" sub-title="" :chart-data="customers.data"
          ...
          <div slot="legend" class="legend-container">
            <div>
            ...
            <div class="button-container">
              <button class="btn btn-primary btn-sm" @click="exportCustomersToExcel">Exportar</button>
            </div>
          </div>
        </chart-card>
      </div>
      ...
      <div class="col-md-6 col-12">
        <chart-card title="Viajes de Choferes" sub-title="" :chart-data="driversChart.data" chart-type="Pie">
          <div slot="legend" class="legend-container">
            <div>
              <label for="monthSelect">Filtrar por mes:</label>
              <select id="monthSelect" v-model="selectedMonth" @change="getDrivers">
                <option v-for="(month, index) in months" :key="index" :value="month.value">{{ month.label }}</option>
              </select>
            </div>
            <div>
              <label for="stateSelect">Filtrar por estado:</label>
            ...
            <div class="button-container">
              <button class="btn btn-primary btn-sm" @click="exportDriversToExcel">Exportar</button>
            </div>
          </div>
        </chart-card>
      </div>
      ...
      <div class="col-md-6 col-12">
        <chart-card title="Ordenes" sub-title="" :chart-data="yearChart.data" :chart-options="yearChart.options">
          ...
          <div slot="legend" class="legend-container">
            ...
            <div>
              <label for="selectYear">Filtrar por año:</label>
              <select id="selectYear" v-model="selectYear" @change="getOrdersYear">
                <option v-for="(year, index) in years" :key="index" :value="year.label">{{ year.label }}</option>
              </select>
            </div>
            <div class="button-container">
              <button type="button" class="btn btn-primary btn-sm" @click="exportOrdersToExcel">Exportar</button>
            </div>
          </div>
        </chart-card>
      </div>
...
export default {
  ...
  methods: {
    // Método para exportar clientes frecuentes a Excel
    async exportCustomersToExcel() {
      // Asegurarse de que los datos estén actualizados antes de exportar
      await this.getCustomers();

      // Exportar los datos filtrados a Excel
      const ws = XLSX.utils.aoa_to_sheet([
        ['Nombre del Cliente', 'Total de Órdenes'],
        ...this.customers.data.labels.map((label, index) => [label, this.customers.data.series[0][index]])
      ]);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Clientes Frecuentes');
      XLSX.writeFile(wb, 'clientes_frecuentes.xlsx');
    },
    // Método para exportar choferes a Excel
    async exportDriversToExcel() {
      // Asegurarse de que los datos estén actualizados antes de exportar
      await this.getDrivers();

      // Exportar los datos filtrados a Excel
      const ws = XLSX.utils.aoa_to_sheet([
        ['Nombre del Chofer', 'Cantidad de Viajes'],
        ...this.driversChart.data.labels.map((label, index) => [label, this.driversChart.data.series[index]])
      ]);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Viajes de Choferes');
      XLSX.writeFile(wb, 'viajes_choferes.xlsx');
    },

    // Método para exportar órdenes a Excel
    async exportOrdersToExcel() {
      // Asegurarse de que los datos estén actualizados antes de exportar
      await this.getOrdersYear();

      // Exportar los datos filtrados a Excel
      const ws = XLSX.utils.aoa_to_sheet([
        ['Mes', 'Total de Órdenes'],
        ...this.yearChart.data.labels.map((label, index) => [label, this.yearChart.data.series[0][index]])
      ]);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Órdenes por Año');
      XLSX.writeFile(wb, 'ordenes_anuales.xlsx');
    },
    async getCustomers() {
      try {
        this.loading = true;  // Habilitar el indicador de carga
        var request = { mes: this.selectMes };
        // Realizar la solicitud POST a /customers-frec
        const response = await axios.post('/customers-frec', request);
        // Usar Vue.set para asegurar la reactividad
        this.$set(this.customers.data, 'labels', []);
        this.$set(this.customers.data, 'series', []);
        const s = [];
    ...
    async getDrivers() {
      try {
        ...
        // Limpiar datos anteriores y usar Vue.set para asegurar la reactividad
        this.$set(this.driversChart.data, 'labels', []);
        this.$set(this.driversChart.data, 'series', []);
    ...
    async getOrdersYear() {
      try {
        ...
        this.$set(this.yearChart.data, 'labels', []);
        this.$set(this.yearChart.data, 'series', []);
        var d = [];
        for (let i = 0; i < response.data.orders.length; i++) {
